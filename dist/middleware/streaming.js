 ...
 */
export function createStreamingResponse(data, options = {}) {
    const { batchSize = 10, delayMs = 0, contentType = 'application/x-ndjson' } = options;
 ...
 */
export function createSSEResponse(dataSource, options = {}) {
 ...
 */
export async function* paginatedDataGenerator(fetchPage, perPage = 20) {
    let page = 1;
    let hasMore = true;
    while (hasMore) {
        try {
