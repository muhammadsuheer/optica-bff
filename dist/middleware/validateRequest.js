...
export function validateRequest(schemas) {
    return async (c, next) => {
        ...
        try {
            ...
            if (schemas.headers) {
                try {
                    const headers = Object.fromEntries(Object.entries(c.req.header()).map(([key, value]) => [key.toLowerCase(), value]));
 ...
 */
export function getValidated(c, type) {
...
export const orderSchema = z.object({
    ...
    billing: z.object({
        ...
        country: z.string().length(2).refine((code) => /^[A-Z]{2}$/.test(code), { message: "Country must be a valid 2-letter ISO code (uppercase)" })
    }),
    shipping: z.object({
        first_name: z.string().min(1),
        last_name: z.string().min(1),
    ...
    currency: z.string().length(3).refine((code) => /^[A-Z]{3}$/.test(code), { message: "Currency must be a valid 3-letter ISO code (uppercase)" }).default('USD'),
