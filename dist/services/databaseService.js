 ...
 */
export async function checkDatabaseHealth(forceCheck = false) {
    const now = Date.now();
 ...
 */
export async function executeWithFallback(operation, fallback) {
    try {
        const startTime = Date.now();
...
export const productOperations = {
    ...
    async getMany(options = {}) {
        return executeWithFallback(async () => {
            const { page = 1, limit = 20, status = 'publish', search, orderBy = 'created_at', orderDirection = 'desc' } = options;
...
export const orderOperations = {
    ...
    async getMany(options = {}) {
        return executeWithFallback(async () => {
            const { page = 1, limit = 20, status, customerId, orderBy = 'date_created', orderDirection = 'desc' } = options;
            const from = (page - 1) * limit;
            const to = from + limit - 1;
            let query = supabaseClient
                .from('orders')
...
export const analyticsOperations = {
     ...
     * Log a metric
     */
    async logMetric(name, value, tags) {
        const result = await executeWithFallback(async () => {
            const { error } = await supabaseClient
                .from('metrics')
                .insert({
...
export default {
    ...
    // Product operations
    getProduct: (id) => productOperations.getById(id),
    getProducts: (options) => productOperations.getMany(options),
    searchProducts: (query, limit) => productOperations.search(query, limit),
    createProduct: (product) => productOperations.upsert(product),
    updateProduct: (id, updates) => productOperations.upsert({ id, ...updates }),
    deleteProduct: (id) => productOperations.delete(id),
    // Order operations
    getOrder: (id) => orderOperations.getById(id),
    getOrders: (options) => orderOperations.getMany(options),
    createOrder: (order) => orderOperations.upsert(order),
    ...
    calculateOrderTotals: async (lineItems) => {
        try {
            ...
            for (const item of lineItems) {
                ...
                const currentPrice = item.variation_id
                    ? product.variations?.find((v) => v.id === item.variation_id)?.price || product.price
                    : product.price;
    ...
    updateOrderStatusWithHistory: async (id, newStatus, note, userId) => {
        try {
            const order = await orderOperations.getById(id);
            if (!order) {
    ...
    updateInventory: async (productId, quantityChange, operation = 'decrease') => {
        try {
    ...
    reserveInventory: async (lineItems, orderId) => {
        try {
            const reservations = [];
            for (const item of lineItems) {
                const product = await productOperations.getById(item.product_id);
    ...
    releaseInventory: async (lineItems) => {
        try {
            for (const item of lineItems) {
    ...
    addToCart: async (data) => {
        try {
            ...
            const existingItemIndex = cart.items.findIndex((item) => item.product_id === productId && item.variation_id === variationId);
            ...
            cart.totals.items_count = cart.items.reduce((sum, item) => sum + item.quantity, 0);
    ...
    updateCartItem: async (data) => {
        try {
            ...
            const itemIndex = cart.items.findIndex((item) => item.product_id === productId && item.variation_id === variationId);
            if (itemIndex === -1) {
    ...
    removeFromCart: async (cartId, cartType, productId, variationId) => {
        try {
            // Get existing cart
            const cart = await getCart(cartId, cartType);
            if (!cart) {
    ...
    clearCart: async (cartId, cartType) => {
        try {
            // Get existing cart
            const cart = await getCart(cartId, cartType);
            if (!cart) {
    ...
    getCartTotals: async (cartId, cartType) => {
        try {
            const cart = await getCart(cartId, cartType);
    ...
    calculateCartTotals: async (items) => {
        let subtotal = 0;
        let items_count = 0;
