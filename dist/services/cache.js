...
export const CACHE_KEYS = {
    // Product caching
    PRODUCT_LIST: (filters, page) => `products:list:${btoa(filters)}:${page}`,
    PRODUCT_DETAIL: (id) => `products:detail:${id}`,
    PRODUCT_VARIANTS: (id) => `products:${id}:variants`,
    // Search caching
    SEARCH_RESULTS: (query, filters) => `search:${btoa(query)}:${btoa(filters)}`,
    SEARCH_SUGGESTIONS: (query) => `search:suggestions:${btoa(query)}`,
    // Cart caching
    CART_CONTENTS: (sessionId) => `cart:${sessionId}`,
    CART_TOTALS: (sessionId) => `cart:${sessionId}:totals`,
    // Order caching
    ORDER_DETAIL: (id) => `orders:detail:${id}`,
    ORDER_STATUS: (id) => `orders:${id}:status`,
    ORDER_LIST: (filters, page) => `orders:list:${btoa(filters)}:${page}`,
...
export class CacheService {
    ...
    memoryTTL = 300000; // 5 minutes in memory max
    constructor() {
    ...
    async getStats() {
        try {
            ...
            const { data: dbData, error } = await supabase.getClient()
                .from('cache_index')
            ...
            const totalHits = dbData?.reduce((sum, item) => sum + item.hit_count, 0) || 0;
            const hitRatio = totalHits > 0 ? totalHits / (totalHits + databaseSize) : 0;
            const topKeys = dbData?.map((item) => ({
                key: item.cache_key,
                hits: item.hit_count
            })) || [];
            return {
    ...
    setMemoryCache(key, value, ttlMs, tags) {
    ...
    async updateCacheStats(key) {
        try {
            await supabase.getClient()
                .from('cache_index')
                .update({
                hit_count: supabase.getClient().raw('hit_count + 1'),
                accessed_at: new Date().toISOString()
            })
                .eq('cache_key', key);
        }
            ...
            try {
                ...
                if (!error && data) {
                    for (const item of data) {
                        results.set(item.cache_key, item.data);
                        // Store in memory cache
                        this.setMemoryCache(item.cache_key, item.data, Math.min(this.memoryTTL, new Date(item.expires_at).getTime() - Date.now()), item.tags || []);
                    }
                }
            }
            catch (error) {
...
export async function invalidateCacheOnWebhook(topic, resourceId) {
    const tagsToInvalidate = [];
    switch (topic) {
        case 'product.created':
        case 'product.updated':
            tagsToInvalidate.push(`product:${resourceId}`, 'products:list', 'search:results', 'search:suggestions');
            break;
        case 'product.deleted':
            tagsToInvalidate.push(`product:${resourceId}`, 'products:list', 'search:results');
            break;
        case 'order.created':
        case 'order.updated':
            tagsToInvalidate.push(`order:${resourceId}`, 'orders:list');
            break;
        case 'order.deleted':
            tagsToInvalidate.push(`order:${resourceId}`, 'orders:list');
            break;
