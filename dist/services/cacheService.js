...
class EdgeCacheService {
    ...
    async get(key) {
        ...
        try {
            ...
            if (memoryEntry && memoryEntry.expires > Date.now()) {
                ...
                return this.deserializeValue(memoryEntry.data, memoryEntry.compressed);
            }
            // 2. Check KV cache
            const kvEntry = await kvClient.get(key);
            if (kvEntry && kvEntry.expires > Date.now()) {
                ...
                return this.deserializeValue(kvEntry.data, kvEntry.compressed);
            }
            // 3. Cache miss
            this.stats.misses++;
    ...
    async set(key, value, options = {}) {
        try {
            ...
            tags = [], compress = false, maxSize = MAX_CACHE_SIZE } = options;
     ...
     */
    async warm(key, generator, options = {}) {
        try {
    ...
    // Private methods
    setMemoryCache(key, entry, ttlMs) {
    ...
    deserializeValue(data, compressed) {
        try {
...
export const CACHE_KEYS = {
    PRODUCTS: {
        LIST: (filters, page) => `products:list:${JSON.stringify(filters)}:${page}`,
        SINGLE: (id) => `products:${id}`,
        SEARCH: (query, limit) => `products:search:${query}:${limit}`,
        CATEGORIES: () => 'products:categories',
        POPULAR: (limit) => `products:popular:${limit}`
    },
    CART: {
        SESSION: (sessionId) => `cart:session:${sessionId}`,
        USER: (userId) => `cart:user:${userId}`,
        TOTALS: (cartId, type) => `cart:totals:${type}:${cartId}`
    },
    ORDERS: {
        SINGLE: (id) => `orders:${id}`,
        LIST: (filters, page) => `orders:list:${JSON.stringify(filters)}:${page}`,
        STATUS: (id) => `orders:${id}:status`
    },
    CUSTOMERS: {
        SINGLE: (id) => `customers:${id}`,
        LIST: (filters, page) => `customers:list:${JSON.stringify(filters)}:${page}`
    },
    HEALTH: {
