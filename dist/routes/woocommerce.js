 ...
 */
woocommerce.post('/sync/products', requireRole(['admin', 'service_role']), validateRequest({ body: syncProductsSchema }), async (c) => {
    try {
        const { page, per_page, force } = getValidated(c, 'body');
 ...
 */
woocommerce.post('/sync/orders', requireRole(['admin', 'service_role']), validateRequest({ body: syncOrdersSchema }), async (c) => {
    try {
        const { page, per_page, after, before } = getValidated(c, 'body');
 ...
 */
woocommerce.post('/webhook', validateRequest({ body: webhookEventSchema }), async (c) => {
    try {
        const event = getValidated(c, 'body');
 ...
 */
woocommerce.get('/status', requireRole(['admin', 'service_role']), async (c) => {
    try {
 ...
 * Fetch products from WooCommerce API
 */
async function fetchWooCommerceProducts(page, per_page) {
 ...
 * Fetch orders from WooCommerce API
 */
async function fetchWooCommerceOrders(page, per_page, after, before) {
 ...
 */
function transformWooProduct(wcProduct) {
    return {
        wc_id: wcProduct.id,
        name: wcProduct.name,
        slug: wcProduct.slug,
 ...
 */
function transformWooOrder(wcOrder) {
    return {
        wc_id: wcOrder.id,
 ...
 */
async function handleProductWebhook(event) {
    switch (event.action) {
        case 'created':
        case 'updated':
