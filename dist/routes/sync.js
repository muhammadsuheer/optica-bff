 ...
 */
sync.post('/full', validateRequest({ body: fullSyncSchema }), async (c) => {
    try {
        const { resources = ['products', 'orders'], force, batch_size } = getValidated(c, 'body');
 ...
 */
sync.post('/cache/invalidate', validateRequest({ body: cacheInvalidateSchema }), async (c) => {
    try {
        const { keys, patterns, clear_all } = getValidated(c, 'body');
 ...
 */
sync.post('/health-check', async (c) => {
    try {
 ...
 */
async function syncResource(resource, options = {}) {
 ...
 * Sync categories
 */
async function syncCategories(options) {
    // Mock implementation
    return {
        success: true,
        synced: 0,
 ...
 */
async function updateLastSyncTimes() {
