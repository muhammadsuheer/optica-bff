...
const RATE_LIMIT_RESPONSE_TEMPLATE = {
    success: false,
    error: {
...
export class GlobalRateLimiter {
    cacheService;
    prefix = 'rl:'; // Shorter prefix for better Redis memory usage
    headerCache = new Map();
     ...
     */
    async checkAndIncrement(key, windowMs, limit) {
        try {
     ...
     */
    async checkCustomLimit(identifier, requests, windowMs, category = 'custom') {
        try {
            const key = `${this.prefix}${category}:${identifier}`;
     ...
     */
    async getStatus(category, identifier) {
        try {
            const config = GLOBAL_RATE_LIMITS[category];
            const key = `${this.prefix}${category}:${identifier}`;
            const current = await this.cacheService.get(key) || 0;
     ...
     */
    async clearLimit(category, identifier) {
        try {
            const key = `${this.prefix}${category}:${identifier}`;
 ...
 */
export function createGlobalRateLimiter(cacheService) {
...
export const rateLimitMiddleware = {
    general: (cacheService) => createGlobalRateLimiter(cacheService).middleware('general'),
    auth: (cacheService) => createGlobalRateLimiter(cacheService).middleware('auth'),
    cart: (cacheService) => createGlobalRateLimiter(cacheService).middleware('cart'),
    products: (cacheService) => createGlobalRateLimiter(cacheService).middleware('products'),
    health: (cacheService) => createGlobalRateLimiter(cacheService).middleware('health'),
    search: (cacheService) => createGlobalRateLimiter(cacheService).middleware('search'),
};
