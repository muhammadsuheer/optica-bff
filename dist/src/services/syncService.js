...
export class SyncService {
    ...
    async initialSync(options = {}) {
        const { batchSize = 50, parallelBatches = 3, resumeFromCheckpoint = true, includeCustomFields = true, syncType = 'products', forceFullSync = false } = options;
        if (this.isRunning) {
        ...
        try {
            ...
            while (checkpoint.page <= checkpoint.totalPages) {
                ...
                for (let i = 0; i < parallelBatches && checkpoint.page <= checkpoint.totalPages; i++) {
                    ...
                    batchPromises.push(this.processBatch(page, batchSize, syncType, includeCustomFields));
                }
     ...
     */
    async processBatch(page, batchSize, syncType, includeCustomFields) {
        try {
            switch (syncType) {
                case 'products':
     ...
     */
    async processBatchProducts(page, batchSize, includeCustomFields) {
        const result = { processed: 0, created: 0, updated: 0, skipped: 0, errors: [] };
        try {
     ...
     */
    async processBatchCategories(page, batchSize) {
        const result = { processed: 0, created: 0, updated: 0, skipped: 0, errors: [] };
        try {
     ...
     */
    async processBatchOrders(page, batchSize) {
        const result = { processed: 0, created: 0, updated: 0, skipped: 0, errors: [] };
     ...
     */
    async processBatchCustomers(page, batchSize) {
        const result = { processed: 0, created: 0, updated: 0, skipped: 0, errors: [] };
     ...
     */
    async createProductInDatabase(product) {
     ...
     */
    async getTotalItemCount(syncType) {
        try {
            switch (syncType) {
                case 'products':
                    return 1000;
    ...
    async saveCheckpoint(checkpoint) {
        try {
            await this.cacheService.set(`sync:checkpoint:${checkpoint.syncType}`, JSON.stringify(checkpoint), 86400 // 24 hours
            );
        }
        catch (error) {
     ...
     * Invalidate related caches
     */
    async invalidateRelatedCaches(syncType) {
        try {
            switch (syncType) {
                case 'products':
    ...
    async getLastSyncTime() {
        try {
            ...
            return lastSyncTime ? new Date(lastSyncTime) : new Date(Date.now() - 24 * 60 * 60 * 1000);
        }
        catch (error) {
