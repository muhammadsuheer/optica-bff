...
export class ProductService {
    ...
    async getProducts(params = {}) {
        ...
        const { page = 1, perPage = 20, search, categories, orderBy = 'DATE', order = 'DESC' } = params;
        try {
     ...
     */
    async fetchProductsFresh(cacheKey, params, startTime) {
        try {
     ...
     */
    scheduleBackgroundRefresh(cacheKey, params) {
     ...
     */
    async getStockDataBatch(params) {
        try {
     ...
     */
    async transformProductsBatch(wpProducts, stockMap) {
     ...
     */
    transformSingleProduct(wpProduct, stockInfo) {
    ...
    async getAllProducts(options = {}) {
        const { batchSize = 100, // Optimal batch size for GraphQL
        maxConcurrent = 8, // Match Undici agent connections
        useCache = true, onProgress } = options;
        try {
            ...
            for (let i = 0; i < totalPages; i += maxConcurrent) {
                pageGroups.push(Array.from({ length: Math.min(maxConcurrent, totalPages - i) }, (_, j) => i + j + 1));
            }
            let loadedCount = 0;
