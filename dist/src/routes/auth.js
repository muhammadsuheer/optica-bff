...
export function createAuthRoutes(cacheService) {
    const auth = new Hono();
    ...
    auth.post('/login', async (c) => {
        ...
        try {
            ...
            const recentAttempts = await cacheService.get(cacheKey);
            ...
            const refreshToken = await sign({ userId: authResult.user.id, type: 'refresh' }, envConfig.security.JWT_SECRET);
    ...
    auth.post('/register', async (c) => {
        ...
        try {
            ...
            const cachedExists = await cacheService.get(existsCacheKey);
            if (cachedExists) {
                return c.json({
                    success: false,
                    error: {
    ...
    auth.get('/profile', async (c) => {
        ...
        try {
            ...
            if (!user) {
                ...
                const userResult = await fetchWordPressUser(payload.userId);
...
async function fetchWordPressUser(userId) {
