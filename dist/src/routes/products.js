 ...
 */
export function createProductRoutes(cacheService) {
    ...
    products.get('/', validateRequest({ query: productQuerySchema }), async (c) => {
        ...
        try {
            ...
            const order = validatedQuery.order.toUpperCase();
     ...
     */
    products.get('/:id', validateRequest({ params: productParamsSchema }), async (c) => {
        const startTime = Date.now();
     ...
     */
    products.get('/search', validateRequest({ query: searchQuerySchema }), async (c) => {
        const startTime = Date.now();
     ...
     */
    products.get('/bulk', validateRequest({
