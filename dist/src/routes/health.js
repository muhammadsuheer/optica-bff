...
let healthCache = {
    liveness: { timestamp: 0, result: null },
    readiness: { timestamp: 0, result: null },
};
const CACHE_TTL = {
 ...
 */
export function createHealthRoutes(cacheService) {
        ...
        try {
            ...
            const healthCheckPromises = [
                Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Redis timeout')), SERVICE_TIMEOUTS.redis))
                ]).catch(() => false),
                Promise.race([
                    wpClient.healthCheck(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('WordPress timeout')), SERVICE_TIMEOUTS.wordpress))
                ]).catch(() => false),
                Promise.race([
                    wooRestClient.healthCheck(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('WooCommerce REST timeout')), SERVICE_TIMEOUTS.woocommerce))
                ]).catch(() => false),
                Promise.race([
                    wooStoreClient.healthCheck(),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('WooCommerce Store timeout')), SERVICE_TIMEOUTS.store))
                ]).catch(() => false),
