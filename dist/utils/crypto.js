 ...
 */
export async function generateHmacSignature(data, secret, algorithm = 'SHA-256') {
    try {
        const dataBuffer = toArrayBuffer(data);
        const secretBuffer = toArrayBuffer(secret);
        // Import the secret key
        const key = await crypto.subtle.importKey('raw', secretBuffer, { name: 'HMAC', hash: algorithm }, false, ['sign']);
        // Generate signature
 ...
 */
export async function verifyHmacSignature(data, signature, secret, algorithm = 'SHA-256') {
    try {
...
export const webhookVerifiers = {
     ...
     */
    async woocommerce(payload, signature, secret) {
        try {
     ...
     */
    async github(payload, signature, secret) {
        try {
     ...
     */
    async stripe(payload, signature, secret) {
        try {
