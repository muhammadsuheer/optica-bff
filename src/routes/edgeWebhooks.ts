...
interface WebhookJob {
  id: string
  source: 'woocommerce' | 'stripe' | 'github'
  event: string
  payload: any
  timestamp: number
  retryCount: number
 ...
 */
async function enqueueWebhookJob(job: WebhookJob): Promise<boolean> {
  try {
    // Add job to processing queue
    const queueSuccess = await kvClient.set(
      `${WEBHOOK_QUEUE_KEY}:${job.id}`,
      job,
      { ttl: 86400 } // 24 hours
    )
