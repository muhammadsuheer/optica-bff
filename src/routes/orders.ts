...
orders.get('/',
  ...
  async (c) => {
    try {
      ...
      const query = getValidated<z.infer<typeof orderQuerySchema>>(c, 'query')
...
orders.post('/',
  ...
  async (c) => {
    try {
      ...
      const user = getCurrentUser(c)
      const orderData = getValidated<z.infer<typeof orderSchema>>(c, 'body')
...
orders.put('/:id/status',
  ...
  async (c) => {
    try {
      const { id } = getValidated<z.infer<typeof commonSchemas.idParam>>(c, 'params')
      const updateData = getValidated<z.infer<typeof updateOrderStatusSchema>>(c, 'body')
...
orders.post('/:id/cancel',
  ...
  async (c) => {
    try {
      ...
      const cancelledOrder = await databaseService.updateOrderStatusWithHistory(
        id, 
        'cancelled',
