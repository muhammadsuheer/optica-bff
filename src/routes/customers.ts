...
export function createCustomerRoutes(cacheService: CacheService): Hono {
  ...
  customers.get('/', async (c) => {
    ...
    try {
      ...
      // Check cache first
      let cachedResult = await cacheService.get<{ customers: Customer[]; total: number }>(cacheKey);
  ...
  customers.get('/:id', async (c) => {
    ...
    try {
      ...
      // Check Redis cache
      let customer = await cacheService.get<Customer>(memoryCacheKey);
  ...
  customers.get('/:id/addresses', async (c) => {
    ...
    try {
      ...
      let addresses = await cacheService.get<CustomerAddress[]>(cacheKey);
...
async function checkCustomerExists(email: string): Promise<boolean> {
...
async function fetchCustomersFromAPI(query: any): Promise<{ customers: Customer[]; total: number }> {
  // Mock customers fetch
  const mockCustomers: Customer[] = Array.from({ length: query.per_page }, (_, i) => ({
    id: i + 1,
...
async function fetchCustomerFromAPI(customerId: number): Promise<Customer | null> {
...
async function getCustomerActiveOrders(customerId: number): Promise<Order[]> {
  // Mock active orders check
  return [];
}

async function fetchCustomerAddresses(customerId: number): Promise<CustomerAddress[] | null> {
...
async function fetchCustomerOrders(customerId: number, page: number, perPage: number): Promise<{ orders: Order[]; total: number }> {
  // Mock customer orders fetch - would normally call the orders API
  const mockOrders: Order[] = Array.from({ length: Math.min(perPage, 5) }, (_, i) => ({
    id: i + 1,
