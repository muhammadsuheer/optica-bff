...
customers.get('/',
  ...
  async (c) => {
    try {
      const query = getValidated<z.infer<typeof customerQuerySchema>>(c, 'query')
...
customers.get('/:id',
  ...
  async (c) => {
    try {
      ...
      const { id } = getValidated<z.infer<typeof commonSchemas.idParam>>(c, 'params')
...
customers.post('/',
  requireRole(['admin', 'service_role']),
  validateRequest({ body: customerSchema }),
  async (c) => {
    try {
      const customerData = getValidated<z.infer<typeof customerSchema>>(c, 'body')
...
customers.put('/:id',
  ...
  async (c) => {
    try {
      const user = getCurrentUser(c)
      const { id } = getValidated<z.infer<typeof commonSchemas.idParam>>(c, 'params')
      const updateData = getValidated<z.infer<typeof updateCustomerSchema>>(c, 'body')
...
customers.get('/:id/orders',
  ...
  async (c) => {
    try {
      const user = getCurrentUser(c)
      const { id } = getValidated<z.infer<typeof commonSchemas.idParam>>(c, 'params')
      const query = getValidated<z.infer<typeof commonSchemas.pagination>>(c, 'query')
