 ...
 */
export function createReviewRoutes(cacheService: CacheService): Hono {
  ...
  reviews.get(
    ...
    async (c) => {
      ...
      try {
        ...
        // Try cache first
        const cached = await cacheService.get<{ reviews: Review[], total: number }>(cacheKey);
        if (cached) {
  ...
  reviews.get(
    ...
    async (c) => {
      ...
      try {
        ...
        // Try cache first
        const cached = await cacheService.get<Review>(cacheKey);
        if (cached) {
          const responseTime = Date.now() - startTime;
          c.header('X-Response-Time', `${responseTime}ms`);
          c.header('X-Cache-Status', 'hit');
    ...
    async (c) => {
      ...
      try {
        ...
        const existingReview = mockReviews.find(r => 
          r.product_id === data.product_id && r.customer_id === data.customer_id
        );
  ...
  reviews.get(
    '/stats/:product_id',
    validateRequest({ params: z.object({ product_id: z.string().regex(/^\d+$/).transform(Number) }) }),
    async (c) => {
      const startTime = Date.now();
