...
const cartCache = new Map<string, { cart: Cart; expires: number }>();
const CART_CACHE_TTL = 300000; // 5 minutes

export function createCartRoutes(cacheService: CacheService): Hono {
  const cart = new Hono();
...
function getCartId(c: any): string {
...
async function getCartWithCache(cartId: string): Promise<Cart> {
...
async function saveCartWithCache(cartId: string, cart: Cart): Promise<void> {
  // Update memory cache
  cartCache.set(cartId, {
    cart,
...
async function getProductWithCache(productId: number): Promise<any> {
...
async function validateCoupon(code: string, cart: Cart): Promise<{
  success: boolean;
  coupon?: any;
  message?: string;
}> {
