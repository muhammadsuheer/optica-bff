 ...
 */
function getCartIdentifier(c: any): { type: 'user' | 'session', id: string } {
...
cart.post('/items',
  ...
  async (c) => {
    try {
      ...
      const cartId = getCartIdentifier(c)
      const itemData = getValidated<z.infer<typeof addToCartSchema>>(c, 'body')
...
cart.put('/items/:itemId',
  ...
  async (c) => {
    try {
      ...
      const cartId = getCartIdentifier(c)
      const updateData = getValidated<z.infer<typeof updateCartItemSchema>>(c, 'body')
 ...
 */
cart.post('/merge',
  validateRequest({ body: cartSessionSchema }),
  async (c) => {
    try {
      if (!hasPermission(c, 'write:cart')) {
        throw new HTTPException(403, { message: 'Insufficient permissions' })
      ...
      const { session_id } = getValidated<z.infer<typeof cartSessionSchema>>(c, 'body')
