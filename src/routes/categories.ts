 ...
 */
export function createCategoryRoutes(cacheService: CacheService): Hono {
  const categories = new Hono();

  // Helper function to build category tree
  const buildCategoryTree = (categories: Category[], parentId?: number): CategoryTree[] => {
    return categories
  ...
  categories.get(
    ...
    async (c) => {
      ...
      try {
        ...
        // Try cache first
        const cached = await cacheService.get<{ categories: Category[], total: number }>(cacheKey);
        if (cached) {
