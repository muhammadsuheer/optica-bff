    ...
    async start(controller) {
      try {
        ...
        for (let page = 1; page * 50 <= Number(limit); page++) {
          const products = await circuitBreakers.woocommerce.execute(() =>
...
products.get('/', async (c) => {
  try {
    ...
    const options = {
      ...
      search: query.search,
      orderBy: (query.orderBy as 'created_at' | 'updated_at' | 'name' | 'price') || 'created_at',
    ...
    return c.json({
      ...
      message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to fetch products'
    }, 500)
  }
})
...
products.get('/popular', async (c) => {
  try {
    ...
    return c.json({
      success: false,
      error: 'Internal Server Error',
      message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to fetch popular products'
    }, 500)
  }
})
...
products.get('/search', async (c) => {
  try {
    ...
    return c.json({
      success: false,
      error: 'Internal Server Error',
      message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to search products'
    }, 500)
  }
})
...
products.get('/:id', async (c) => {
  try {
    ...
    return c.json({
      success: false,
      error: 'Internal Server Error',
      message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to fetch product'
    }, 500)
  }
})
  ...
  async (c) => {
    try {
      ...
      return c.json({
        success: false,
        error: 'Internal Server Error',
        message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to create/update product'
      }, 500)
    }
  }
)
  ...
  async (c) => {
    try {
      ...
      return c.json({
        success: false,
        error: 'Internal Server Error',
        message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to bulk create/update products'
      }, 500)
    }
  }
)
  ...
  async (c) => {
    try {
      ...
      return c.json({
        success: false,
        error: 'Internal Server Error',
        message: config.nodeEnv === 'development' ? (error as Error).message : 'Failed to delete product'
      }, 500)
    }
  }
)
